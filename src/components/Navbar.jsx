...
export default function Navbar({ toggleSidebar }) {
  ...
  const NavItem = ({ label, page }) => (
    <button
      ...
      className="px-6 py-3 bg-emerald-400 text-black font-bold rounded-full hover:bg-green-700 shadow-md text-sm transition"
    >
      {label}
    </button>
  );
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-20 bg-gray-800 flex items-center px-6 shadow-md z-50 border-b ">
      ...
      <button
        ...
        className="ml-auto bg-emerald-400 w-10 h-10 rounded-full text-black text-xl flex items-center justify-center hover:bg-green-700 transition"
      >
        ☰
      </button>
    </div>
